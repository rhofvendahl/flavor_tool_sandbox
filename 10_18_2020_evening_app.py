...
# TODO: account for if connected subgraph is impossible
    ...
    #
    # # not totally sure these are right (that both min and max are compared to locked)
    ...
    for iteration in range(n_iterations):
        ...
        while True:
            ...
            if len(final_cliques) > 0: # BLACK MAGIC
                ...
                # n_additional_non_clique_other_flavorings = max(n_other_flavorings_actual - len(other_flavorings_so_far_set), 0) # pretty sure there shouldn't be more salts so far than salts actual
                # n_additional_non_clique_foodstuffs = max(n_foodstuffs_actual - len(foodstuffs_so_far_set), 0) # pretty sure there shouldn't be more salts so far than salts actual
                ...
                # print('ADDITIONAL SALTS POOL WEAK SCORE', the_rest_salts[~the_rest_salts['name'].isin(clique_ingredients['name'])]['weak_score'])
                # print('ADDITIONAL SALTS POOL LEN', len(the_rest_salts[~the_rest_salts['name'].isin(clique_ingredients['name'])]))
                ...
                if n_additional_salts_actual > 0:
                    print('N ADDITIONAL SALTS ACTUAL GREATER THAN ZERO')
                ...
                if n_additional_fat_oils_actual > 0:
                    print('N ADDITIONAL FAT OILS GREATER THAN ZERO')
                ...
                if n_additional_foodstuffs_actual > 0:
                    ...
                    print('LOCKED FOODSTUFFS LEN', len(locked_foodstuffs))
                    print('THE REST FOODSTUFFS LEN', len(the_rest_foodstuffs))
                    print('N ADDITIONAL FOODSTUFFS ACTUAL', n_additional_foodstuffs_actual)
                # selected_ingredients = selected_salts.append(selected_fat_oils).append(selected_other_flavorings).append(selected_foodstuffs)
            ...
            for i_1, name_1 in enumerate(selected_names[:-1]):
                for i_2, name_2 in enumerate(selected_names[i_1+1:], i_1+1):
                    ...
                    # length is used for average length, which goes into pairing score
                    # weight is used for degree, which goes into strength and locked scores
                    # if connection[0] != '_':
        ...
        if scoring_method == 'connected':
            ...
            # and also encompasses strength-ness and locked-ness
            # not really sure how this sranges. hopefully (0 - 1) * 3? Hard to test.
            print('CONNECTED CC AND TYPE', type(connected_components), connected_components)
            print('NODES IN GRAPH', selected_g.nodes())
            print('PRESENT INGREDIENTS', present_names)
            print('SELECTED INGREDIENTS', selected_names)
            largest_cc = max(connected_components, key=len)
            largest_subgraph = selected_g.subgraph(largest_cc) # .copy()?
            largest_subgraph_g = nx.Graph(largest_subgraph)

            average_shortest_path_length = nx.average_shortest_path_length(largest_subgraph_g, weight='length')
            average_shortest_path_score = 1 / average_shortest_path_length * 1.5 - 1.25
            # print('DISCONNECTED AVERAGE SHORTEST PATH SCORE', average_shortest_path_score)
            score += average_shortest_path_score * 5
